...
**Since Django autogenerates the migration's for the creation of the tables, there in no explicit creation of the tables.They are created automatically when the project is run for the first time** However if we wanted to create the table manually the sql command is given below
